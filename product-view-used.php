...
<head>
    ...
    <style>
        ...
        .lightbox-close:hover { color: #8b5cf6; } /* Accent color */
...
<script>
  // Build JS images array from PHP variables
  <?php 
    $privacyJsImages = [];
    if (!empty($img1)) $privacyJsImages[] = $img1;
    if (!empty($img2)) $privacyJsImages[] = $img2;
    if (!empty($img3)) $privacyJsImages[] = $img3;
    if (!empty($img4)) $privacyJsImages[] = $img4;
  ?>
  const productImages = [<?php echo implode(',', array_map(function($i){ return "'".htmlspecialchars($i, ENT_QUOTES)."'"; }, $privacyJsImages)); ?>];
  ...
  const mainProductImage = document.getElementById('mainDisplayImage');
  ...
  if (mainProductImage) {
    ...
    mainProductImage.addEventListener('click', function() {
      ...
      // Current src might be absolute url, productImages has relative path or whatever is in src attribute.
      // We look for partial match or exact match.
      // Best to grab attribute 'src' directly if it matches the array logic.
      
      const srcAttr = this.getAttribute('src');
      let idx = productImages.findIndex(img => img === srcAttr);
      if (idx === -1) idx = 0;
  ...
  function updateLightboxImage() {
    ...
    // productImages now contains the full path as used in img src
    const newSrc = productImages[currentLbIndex];
